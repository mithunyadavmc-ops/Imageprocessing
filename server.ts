...
// Do not auto-seed fake jobs on start so dashboard starts clean for user uploads
// seedInitialJobs();
...
async function startServer() {
  ...
  const handleUpload = async (req: Request, res: Response) => {
    try {
      ...
      if (presetKey) {
        initialImageUrl = SAMPLE_VEHICLES.find((p) => p.id === presetKey)?.imageUrl;
      } else if (imageBuffer && imageBuffer.length > 0) {
        const ext = filename.split('.').pop()?.toLowerCase();
        const mimeType = ext === 'png' ? 'image/png' : ext === 'webp' ? 'image/webp' : 'image/jpeg';
        initialImageUrl = `data:${mimeType};base64,${imageBuffer.toString('base64')}`;
      }
